...
\end{figure}
 ...
 In figure \ref{fig:block_sim_t} and \ref{fig:block_sim_beta_sigma10}, we have plotted the parameter $t$  for two different values of $\sigma_t$. We clearly see how increasing the tuning parameter from $\sigma_t = 3$ to $\sigma_t = 10$ decreases the acceptance rate of the Metropolis Hastings step in our MCMC algorithm. Far fewer proposed samples are accepted. In figure \ref{fig:block_sim_beta} and \ref{fig:block_sim_beta_sigma10}, we have plotted the trace plot of parameter $\beta$ for two different values of $\sigma_{\beta}$. Here, we see no discernible difference in the acceptance rate of the Metropolis Hastings step for the different values of the tuning parameter, $\sigma_{\beta} = 0.03$ and $\sigma_{\beta} = 10$. We are unsure of why this is.


\todo[inline]{Oppgaven sier "compare the burn-in and the mixing properties of the single site and block MH-alg. Lite problem: får ikke til å kjøre med sigma t det samme som i single site. Skal jeg da endre på plottene i single site?}
