...
# Importing data
data <-
  as.data.table(read.delim("code/Gaussiandata.txt", header = FALSE))
...
pi_theta_cond <- function(T, eta, Q) {
  return (rgamma(1, shape = ((T / 2)), scale = 1 / (1 + (t(
    eta
  ) %*% Q %*% eta) * 0.5)))
}
...
posterior_dist_MC <-
  Gibbs_alg(n, data, eta_0 = eta, theta_0 = theta)

q <-
  ggplot(data = as.data.table(posterior_dist_MC[1:25000, ]), aes(x = 1:25000))
q <- q + geom_line(aes(y = V1))
q <- q + ylab(unname(TeX(c("$\\theta$")))) +
  ggtitle(unname(TeX(
    "Traceplot for MCMC samples of theta $\\theta$"
  ))) +
  xlab("Iteration")
q
...
posterior_dist_MC_usefull <-
  posterior_dist_MC[10000:nrow(posterior_dist_MC),]
posterior_dist_MC_usefull <-
  as.data.table(posterior_dist_MC_usefull)
...
posterior_dist_MC_usefull.long = melt(
  posterior_dist_MC_usefull,
  id.vars = c("V1", "itteration"),
  measure.vars = c(2:21)
)
...
q <-
  ggplot(post_eta_mcmc_aggregated, aes(x = as.factor(variable), y = mean))
q <- q + geom_point() +
...
pi_theta_given_y <- function(theta, Q, I, y, eta) {
  eta_theta <-
    theta ^ ((T - 2) / 2) * exp(-theta - 0.5 * theta * t(eta) %*% Q %*% eta)
  likelihood <-
    dmvnorm(x = y,
            mean = eta,
            sigma = diag(1, nrow = T, ncol = T))
...
post_theta <-
  (lapply(seq(0.0000, 6, length.out = n), pi_theta_given_y, Q, I, y, eta))
...
q <-
  q + geom_histogram(aes(y = ..density.., colour = "MCMC"), bins = 100)
q <-
  q + geom_point(data = eta,
                 aes(
                   x = eta_grid,
                   y = density_total * 0.9 * 10 ^ 7,
                   colour = "INLA_manualy"
                 ))
...
q <-
  q + geom_line(data = as.data.table(precision), aes(x = x, y = y , colour = "INLA"))
q <-
  q + geom_point(data = data_post,
                 aes(
                   x = x,
                   y = post_theta * 0.25 * 10 ^ 9,
                   colour = "INLA_manually"
                 ))
q <-
...
post_eta_inla_summary <-
  as.data.table(result$summary.random)[, .(t.ID, t.mean, t.0.025quant, t.0.975quant)]
post_eta_inla_summary[, group := "INLA"]

post_eta_total <-
  rbindlist(list(post_eta_mcmc_aggregated, post_eta_inla_summary))

q <-
  ggplot(post_eta_total,
         aes(
